...
echo "javac $JAVAC_OPTIONS  -d mods --module-path mlib${PATH_SEPARATOR}amlib1 --module-source-path src \$(find src/modbar -name \"*.java\")"
$JAVA_HOME/bin/javac $JAVAC_OPTIONS  -d mods --module-path mlib${PATH_SEPARATOR}amlib1 --module-source-path src $(find src/modbar -name "*.java") 2>&1
