...
Currently work in progress. These are the steps we did so far.
...
The application now compiles, tests and runs.

#### Introducing a module
8. We converted `mod.app`into a module by adding a module-info.
9. More byte code enhancement issues. Therefore we declared the main class explicitely in the spring-boot:run-plugin. This avoids letting spring boot search a main class itself. See https://github.com/spring-projects/spring-boot/issues/10647
...
The application now compiles, tests and runs - tests and runs only from the classpath.
