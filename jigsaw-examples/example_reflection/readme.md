...
In modmain, we call methods of both classes statically (only B) and dynamically via reflection (`B` and `InternalB`). 

Various variants of methods (public, private, private static methods are called via reflection). In the `module-info` of `modb` there is no more distinction possible, both exports are `exports private`.
