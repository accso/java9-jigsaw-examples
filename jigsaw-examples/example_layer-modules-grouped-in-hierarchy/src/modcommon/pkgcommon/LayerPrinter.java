...
public final class LayerPrinter {
	public static void printRuntimeInfos(ModuleLayer layer, Object o) throws Exception {
		...
		System.out.println(
				"Layer (" + layerAsString + (layer.equals(ModuleLayer.boot()) ? "), boot layer" : "), not boot layer"));
		...
		System.out.println(o.getClass().getModule().getLayer().modules().stream().map(Module::getName).sorted()
				.collect(Collectors.joining(", ")));
		System.out.println("Layer's configuration with its list of resolved modules:");
		System.out.println(o.getClass().getModule().getLayer().configuration().modules().stream()
				.map(ResolvedModule::name).sorted().collect(Collectors.joining(", ")));
	}
}
