...
do
   echo "javac -Xlint -d mods --module-path mlib${PATH_SEPARATOR}amlib --module-source-path src \$(find src/${modx} -name \"*.java\")"
   $JAVA_HOME/bin/javac -Xlint -d mods --module-path mlib${PATH_SEPARATOR}amlib --module-source-path src $(find src/${modx} -name "*.java")
done

# Compile the rest
echo "javac -Xlint -d mods --module-path mlib${PATH_SEPARATOR}amlib --module-source-path src \$(find src -name \"*.java\" | grep -v mod.x)"
$JAVA_HOME/bin/javac -Xlint -d mods --module-path mlib${PATH_SEPARATOR}amlib --module-source-path src $(find src -name "*.java" | grep -v mod.x)
