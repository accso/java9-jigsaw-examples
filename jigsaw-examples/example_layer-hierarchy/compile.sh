...
do
   echo "javac -d mods --module-path mlib\;amlib -modulesourcepath src \$(find src/${modx} -name \"*.java\")"
   $JAVA_HOME/bin/javac -d mods --module-path mlib\;amlib -modulesourcepath src $(find src/${modx} -name "*.java")
done

# Compile the rest
echo "javac -d mods --module-path mlib\;amlib -modulesourcepath src \$(find src -name \"*.java\" | grep -v mod.x)"
$JAVA_HOME/bin/javac -d mods --module-path mlib\;amlib -modulesourcepath src $(find src -name "*.java" | grep -v mod.x)
