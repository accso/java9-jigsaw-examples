...
echo "javac -d mods --module-path mlib --add-reads modb=modc --add-exports modc/pkgc=modb -modulesourcepath src \$(find src/modb -name \"*.java\")"
$JAVA_HOME/bin/javac -d mods \
    --add-reads modb=modc \
    --add-exports modc/pkgc=modb \
    --add-modules modc \
    --module-path mlib \
    -modulesourcepath src $(find src/modb -name "*.java")
...
$JAVA_HOME/bin/javac -d mods \
    ...
    --add-modules modc \
    --module-path mlib \
    -modulesourcepath src $(find src/modmain -name "*.java")
